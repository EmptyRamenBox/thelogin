<template>
  <nav class="flex bg-indigo-700 text-white items-center h-16 px-8 justify-between">
    <div>
      <router-link class="logo font-bold text-xl tracking-tight" to="/">TheLogin</router-link>
    </div>
    <div>
      ...
      <button
        ...
        @click="logOut"
      >Logout</button>
      <router-link class="p-2 mx-1 hover:text-indigo-700 hover:bg-white" to="/public">Public</router-link>
      <router-link
        v-if="user"
        class="p-2 mx-1 hover:text-indigo-700 hover:bg-white"
        to="/private"
